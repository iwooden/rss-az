...
def expand_node(
    ...
    Args:
        node: MCTSNode to expand (modified in place).
        policy_priors: NN policy output, shape (action_dim,).
        legal_mask: Binary mask, shape (action_dim,). 1.0 for legal.
        num_players: Number of players in the game.
        default_value: NN value for this node, shape (num_players,).
    """
    cdef int action_dim = legal_mask.shape[0]
    cdef int i, p, idx
    ...
    # Fill legal_actions and priors in one pass
    idx = 0
    for i in range(action_dim):
        if legal_mask[i] > 0.0:
            a_view[idx] = i
            p_view[idx] = policy_priors[i]
            idx += 1
    ...
    # Broadcast default_value into value_sums rows
    for i in range(n):
        for p in range(num_players):
            vs_view[i, p] = default_value[p]
