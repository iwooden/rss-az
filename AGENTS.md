...
- `core/token_data.{pxd,pyx}`: token extraction. Engine-side tokens are
  `num_players + 54` rows, `TOKEN_DIM = 92`; 4 learned pass anchors are appended
...
- `nn/transformer.py`: active model. Token-based entity readout, unified dense
  ...
  learned `company_id_embed` / `corp_id_embed` / `player_id_embed` inferred
  from token row order; entity ID one-hots are not emitted. Company ownership
