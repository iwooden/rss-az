...
- **Model.** `nn/transformer.py::RSSTransformerNet` — pre-RMSNorm + SwiGLU, entity-readout heads, per-phase dispatch, per-row logit gather. `TransformerConfig` defaults: 3p, d_model=128, 10 layers, 2 heads, ~2.37M params. `forward(x, phase_ids, action_ids, n_legals, phase_indices=None) → (policy_logits[B, K_MAX], values[B, N])`.
