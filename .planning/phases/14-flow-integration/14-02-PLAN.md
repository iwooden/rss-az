...
Note: `GamePhases.PHASE_CLOSING` is the correct pattern. The `GamePhases` enum is already imported in `phases/acquisition.pyx` via `from core.data cimport GamePhases`. See `phases/wrap_up.pyx:184` for the same pattern used with `GamePhases.PHASE_ACQUISITION`.
