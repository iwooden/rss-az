...
**INV-21 Clarification:** Per CONTEXT.md, shares are "fungible - no special President's Share card treatment." When a player buys a share from a receivership corp, they automatically become president because:
1. `_check_receivership()` sees total_player_shares > 0, clears receivership flag
2. `_check_presidency()` finds the buyer has the most shares (only player with shares), sets them as president

No special "president share" handling is needed - the buyer simply becomes president by having the most shares.
</objective>
...
Per CONTEXT.md: "Tie-breaking: Current president keeps it when shares are equal"

The tie-breaking logic must preserve incumbent advantage correctly. The key insight is:
- If a player has MORE shares than current max, they become the new leader
- If a player TIES the current max AND they are the incumbent, they keep presidency
- The incumbent advantage only applies when shares are EQUAL, not when someone has more
...
**Why the two-pass approach is correct:**
1. First pass finds the maximum share count among all players
2. Second pass checks if incumbent has that max count (if so, they keep it)
3. Only if incumbent doesn't have max shares do we find the first player with max shares

This ensures incumbent advantage is properly preserved - they only lose presidency if another player has STRICTLY MORE shares.
  </action>
  <verify>
Build succeeds: `python setup.py build_ext --inplace`
  </verify>
  <done>
`_check_receivership()` and `_check_presidency()` helpers exist in invest.pyx with correct tie-breaking logic.
  </done>
</task>
...
This handles INV-21: buying a share from receivership corp means at least one player now owns a share, so `_check_receivership` will clear the flag, and `_check_presidency` will set the buyer as president (they have the most shares - they're the only holder).
  </action>
  <verify>
Build succeeds: `python setup.py build_ext --inplace`
...
Note: issued_shares must equal bank_shares + player_shares. With bank_shares=2 and player_shares=2, issued_shares=4.
...
Test count increased (should be ~35+ tests now).
  ...
  <done>
Tests cover INV-18 through INV-27. All tests pass. Fixture has correct issued_shares=4.
  </done>
</task>
...
- INV-19: Incumbent keeps on tie (test: test_presidency_incumbent_keeps_on_tie) - using correct two-pass algorithm
- INV-20: Receivership when all player shares = 0 (test: test_receivership_when_all_shares_sold)
- INV-21: Buying from receivership makes buyer president (test: test_receivership_exit_on_buy) - no special share handling needed
