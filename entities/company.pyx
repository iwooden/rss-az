...
from core.data cimport (
    GameConstants, COMPANY_INCOME_DIVISOR,
...
cdef class Company:
    ...
    cpdef int get_adjusted_income(self, GameState state):
        ...
        """
        return <int>lround(state._data[self._income_offset] * COMPANY_INCOME_DIVISOR)

    cpdef void set_adjusted_income(self, GameState state, int income):
        """Set company's adjusted income."""
        state._data[self._income_offset] = <float>income / COMPANY_INCOME_DIVISOR
