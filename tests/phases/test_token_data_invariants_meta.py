...
# MarketInfo token layout: attention mask + 27 slot prices + 27 availability.
_MI_AVAIL_BASE = 28

# ---- MarketInfo: slot prices ---------------------------------------------

def corrupt_market_slot_price(buf):
    buf[MARKET_INFO_TOK, 1 + 5] = 0.0              # real value ≈ 9/75
...
def corrupt_market_info_tail(buf):
    buf[MARKET_INFO_TOK, _MI_AVAIL_BASE + 27] = 1.0  # tail past 55 slots

# ---- Company token: static data ------------------------------------------

def corrupt_company_face_value(buf):
    buf[COMPANY_BASE_TOK + 5, 3] = 0.0             # FACE_VALUE offset

def corrupt_company_low_high_diff(buf):
    buf[COMPANY_BASE_TOK + 5, 5] = 0.0             # LOW_HIGH_DIFF offset

def corrupt_company_base_income(buf):
    buf[COMPANY_BASE_TOK + 5, 6] = 0.0             # BASE_INCOME offset

def corrupt_company_stars(buf):
    buf[COMPANY_BASE_TOK + 5, 7] = 0.0             # STARS offset

# ---- Company token: dynamic (adj_income / is_selected / at_* / owner_*) --

def corrupt_company_adj_income(buf):
    buf[COMPANY_BASE_TOK + 5, 8] = 9.99            # ADJ_INCOME mismatch

def corrupt_company_is_selected(buf):
    buf[COMPANY_BASE_TOK + 0, 1] = 1.0             # active_company=-1 so must be 0

def corrupt_company_at_auction_wrong(buf):
    buf[COMPANY_BASE_TOK + 5, 10] = 0.0            # company 5 IS at AUCTION

def corrupt_company_at_removed_false(buf):
    buf[COMPANY_BASE_TOK + 0, 9] = 1.0             # company 0 is NOT at REMOVED

def corrupt_company_at_revealed_false(buf):
    buf[COMPANY_BASE_TOK + 0, 11] = 1.0            # nothing at REVEALED initially

def corrupt_company_owner_corp_false(buf):
    buf[COMPANY_BASE_TOK + 0, 14] = 1.0            # no corp owns anything initially

def corrupt_company_owner_player_false(buf):
    buf[COMPANY_BASE_TOK + 0, 22] = 1.0            # no player owns anything initially

def corrupt_company_owner_fi_false(buf):
    buf[COMPANY_BASE_TOK + 0, 27] = 1.0            # FI owns nothing initially

def corrupt_company_tail(buf):
    buf[COMPANY_BASE_TOK + 0, 28] = 1.0            # padding past TW_COMPANY must stay 0

# ---- FI ------------------------------------------------------------------

def corrupt_fi_cash(buf):
    buf[FI_TOK, 1] = 0.0                           # FI cash = 4 initially
...
def corrupt_global_phase_onehot(buf):
    buf[GLOBAL_INFO_TOK, 1] = 0.0                  # INVEST dp=0 bit set

def corrupt_global_coo_onehot(buf):
    buf[GLOBAL_INFO_TOK, 12] = 0.0                 # CoO level 1 → slot 0 of COO region

def corrupt_global_end_card(buf):
    buf[GLOBAL_INFO_TOK, 19] = 1.0                 # end_card unflipped initially

def corrupt_global_cards_remaining(buf):
    buf[GLOBAL_INFO_TOK, 20] = 0.0                 # cards_remaining = 17 != 0

def corrupt_global_num_players(buf):
    buf[GLOBAL_INFO_TOK, 21] = 0.0                 # 3p → first num_players slot set

def corrupt_global_info_tail(buf):
    buf[GLOBAL_INFO_TOK, 24] = 1.0                 # tail past 24 slots

# ---- Invest token (in-phase) --------------------------------------------

def corrupt_invest_passes(buf):
    buf[INVEST_TOK, 1] = 1.0                       # consecutive_passes=0

def corrupt_invest_tail(buf):
    buf[INVEST_TOK, 2] = 1.0                       # tail past 2 slots
...
def corrupt_corp_inactive_active_flag(buf):
    buf[CORP_BASE_TOK + 0, 2] = 1.0                # OFF_ACTIVE; corp 0 inactive

def corrupt_corp_inactive_price_idx(buf):
    buf[CORP_BASE_TOK + 0, 8] = 1.0                # inactive → price_idx zeros
...
def corrupt_player_cash(buf):
    buf[PLAYER_BASE_TOK + 0, 8] = 0.0              # OFF_CASH; player 0 cash=30
...
def corrupt_active_corp_pending_move(buf):
    buf[CORP_BASE_TOK + 0, 36] = 0.0               # active corp pending move


def corrupt_active_corp_raw_revenue(buf):
    buf[CORP_BASE_TOK + 0, 41] = 0.0               # active corp raw revenue


def corrupt_active_corp_synergy(buf):
    buf[CORP_BASE_TOK + 0, 42] = 1.0               # active corp synergy income


def corrupt_active_corp_coo_cost(buf):
    buf[CORP_BASE_TOK + 0, 43] = 1.0               # active corp CoO cost
...
def corrupt_acq_price_info_max_offset(buf):
    buf[ACQ_PRICE_INFO_TOK, 1] = 0.0


def corrupt_acq_price_info_fi_flag(buf):
    buf[ACQ_PRICE_INFO_TOK, 2] = 1.0


def corrupt_acq_price_info_total_synergies(buf):
    buf[ACQ_PRICE_INFO_TOK, 3] = 0.0
