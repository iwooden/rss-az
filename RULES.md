...
- V3 cross-president offers (to another player or a corporation with a different president) require the company's full maximum price. A buying corporation that cannot afford that price cannot select the company; corporations with no affordable legal targets are excluded from selection. Locked acquisition proceeds cannot fund an offer. The price must still exceed that player's highest rejected price for the company, so rejecting a maximum-price offer prevents further offers from that player for that company during the phase. Same-president purchases retain their full legal price range. OS's face-value discount applies only to FI purchases. This restriction reduces speculative low offers that are routinely rejected.
