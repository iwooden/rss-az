...
Token order (matches ``nn/transformer-v3.py``):
...
V3 has width 98: corp slots 54..56 hold actor buys/4, sells/4,
and buys>=1 && sells>=1, with the relation tail at 57. Player slot 14
reports any-corp round-trip history, retained until the next turn.
See token-data-v3.md for the input contract.

V3 per-token feature layouts (max width 98, pinned by the Corp token):
  ...
  Corp   (98):  attn_mask (1) + is_selected (1) + active (1) +
                ...
                actual_pending_price_move (1) + cash (1) + acq_proceeds (1) +
                ...
                actor buys/sells/round-trip (3) + relational tail: president_id onehot (5) +
  ...
  Divd    (53): attn_mask (1) + actual dividend index deltas (26)
                + resolved dollar share prices (26, / CASH_DIVISOR)
...
from entities.corp cimport (
    ...
    _simulate_dividend_resolved_price_move,
    _simulate_float,
)
...
# Fixed token layout constants (see nn/transformer-v3.py). These match the
...
cpdef object get_token_widths(int max_players):
    ...
    cdef int token_dim = <int>TokenDataSize.TOKEN_DIM
    ...
    for i in range(NUM_CORPS):
        w[tok] = <unsigned char>token_dim  # Corp is the widest token.
        tok += 1
...
cdef void _fill_corp_token(
    ...
    if active:
        ...
        buffer[tok, OFF_PENDING_MOVE] = <float>_simulate_dividend_resolved_price_move(state, corp_id, 0) / IMPACT_DIVISOR
...
cdef void _fill_dividend_token(
    ...
    cdef int OFF_PRICE     = OFF_IMPACT + MAX_DIVIDEND  # resolved dollar prices

    cdef int active_corp = <int>state._data[LAYOUT.turn_offset + TURN_OFFSETS.active_corp]
    cdef int amount, price_move, current_index
    ...
    current_index = corp_price_index(state, active_corp)
    for amount in range(MAX_DIVIDEND):
        price_move = _simulate_dividend_resolved_price_move(state, active_corp, amount)
        buffer[tok, OFF_IMPACT + amount] = <float>price_move / IMPACT_DIVISOR
        buffer[tok, OFF_PRICE + amount] = <float>MARKET_PRICES[current_index + price_move] / CASH_DIVISOR
