...
documented in ``token-data-v3.md`` and matches the order expected by
``nn/transformer-v3.py``.
"""
...
# to ``nn/transformer-v3.py``). All tokens are zero-padded to this width.
...
# ``_fill_*_token`` helper in ``token_data_v3.pyx`` writes exactly
