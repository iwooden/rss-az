...
class TransformerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        assert self.price_slot_fourier_bands > 0, (
            "price_slot_fourier_bands must be positive, "
...
class RSSTransformerNet(nn.Module):
    ...
    def __init__(self, cfg: TransformerConfig) -> None:
        ...
        self._global_info_feature_start = self._token_feature_start
        ...
        # ``2^k * 2π`` for k in [0, price_slot_fourier_bands).
    ...
    def forward(
        ...
        Args:
            ...
            relations: Sparse directed relation coordinates ``(batch,
                MAX_ATTENTION_RELATION_EDGES, ATTENTION_RELATION_COORD_WIDTH)``
                uint8. Rows are ``(relation_id, query_token, key_token)``
        ...
        if tuple(relations.shape) != expected_sparse_rel_shape:
            raise AssertionError(
                f"relations shape must be {expected_sparse_rel_shape} for "
                f"sparse relation coordinates; "
    ...
    def pass_action_logit_abs(
        ...
        Used as a TB diagnostic to detect logit-scale drift between slot-0
        pass/no-op actions and the remaining legal actions in the same phase.
